...
describe('WebSocket game engine full-flow integration', () => {
  ...
  test('submit_answer (correct) returns answer_result with correct:true and positive points; broadcasts score-update', async () => {
    ...
    // Host answered Q1 correctly (first correct answer), so streak=1 and multiplier=1
    expect(typeof answerResult.streak).toBe('number');
    expect(answerResult.streak).toBeGreaterThanOrEqual(1);
    expect(typeof answerResult.multiplier).toBe('number');
    expect(answerResult.multiplier).toBeGreaterThanOrEqual(1);
  ...
  test('when all players submit, question_end (with correctAnswer + leaderboard) and leaderboard_update are broadcast immediately; timer is cleared', async () => {
    ...
    for (const entry of lbUpdate.leaderboard) {
      expect(typeof entry.multiplier).toBe('number');
    }
  ...
  test('streak is isolated per player: player A streak does not affect player B', async () => {
    // Correct answers per FIXED_QUESTIONS: Q1→1, Q2→2, Q3→2
    // Host answers all 3 correctly; Player1 answers all 3 incorrectly (index 0)
    ...
    for (let i = 0; i < 3; i++) {
      // Set up all listeners BEFORE submitting to avoid missing events
      const hostAnswerResultP = waitForMessage(hostWs, (m) => m.type === 'answer_result');
      const playerAnswerResultP = waitForMessage(playerWs, (m) => m.type === 'answer_result');
      const questionEndP = waitForMessage(hostWs, (m) => m.type === 'question_end');

      wsSend(hostWs, { type: 'submit_answer', answer: correctAnswers[i] });
      wsSend(playerWs, { type: 'submit_answer', answer: wrongAnswer });

      const [hostResult, playerResult] = await Promise.all([hostAnswerResultP, playerAnswerResultP]);

      // Per-event isolation: host streak grows, player1 streak stays 0
      expect(hostResult.correct).toBe(true);
      expect(hostResult.streak).toBe(i + 1);

      expect(playerResult.correct).toBe(false);
      expect(playerResult.streak).toBe(0);
    ...
    const playerEntry = gameOver.leaderboard.find((e) => e.name === 'Player1');
